resource "aws_subnet" "private" {
  ...
  tags              = merge(map("Name", "${var.tag_name_identifier}-private-${element(var.list_of_aws_az, count.index)}"),var.global_tags,var.other_private_subnet_tags)
...
resource "aws_route" "to_nat_gw" {
  count                  = var.number_of_private_subnets * (var.associate_nat_gateway_with_private_route_table ? 1 : 0)
  route_table_id         = aws_route_table.private.*.id[count.index]
...
resource "aws_vpc_endpoint_route_table_association" "private_s3" {
  count           = var.number_of_private_subnets * (var.associate_s3_endpoint_with_private_route_table ? 1 : 0)
...
# Enable VPN route propogation to private route tables
resource "aws_vpn_gateway_route_propagation" "private-rt" {
  count           = var.number_of_private_subnets
  route_table_id  = aws_route_table.private.*.id[count.index]
